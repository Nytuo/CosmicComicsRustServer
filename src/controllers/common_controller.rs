...
pub async fn get_color(
    ...
    let serializable_palette: SerializableRgb = palette
        .into_iter()
        .next()
        .map(SerializableRgb::from)
        .unwrap_or(SerializableRgb { r: 0, g: 0, b: 0 });
    let rgb_array = vec![
        serializable_palette.r,
        serializable_palette.g,
        serializable_palette.b,
    ];
    (StatusCode::OK, axum::Json(serde_json::json!(rgb_array))).into_response()
}
...
pub async fn get_palette_color(
    ...
    let mut img = match request.headers().get("img").and_then(|v| v.to_str().ok()) {
