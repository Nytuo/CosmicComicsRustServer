...
pub fn database_routes(state: Arc<tokio::sync::Mutex<AppState>>) -> Router {
    Router::new()
        ...
        .route("/DB/update/{token}/{dbName}/{colName}/{value}/{id}", get(update_db))
        ...
        .route("/DB/delete/truedelete/{token}/{dbName}/{id}", get(true_delete_db))
        .route("/DB/lib/delete/{token}/{id}", get(delete_lib))
        .route("/DB/get/{token}/{dbName}", get(get_db))
        .with_state(state)
}
